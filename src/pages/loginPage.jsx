    ...
    return (
        <div className="w-full h-full bg-[url('/login-bg.jpg')] bg-cover bg-no-repeat flex justify-center items-center">
            ...
            <div className="w-[400px] h-[500px] backdrop-blur-md shadow-2xl shadow-white rounded-xl flex flex-col p-4">
                ...
                <button  disabled={loading} className="w-full h-[50px] bg-blue-950 mt-10 text-white rounded-lg" onClick={handleLogin}>
                    {
                        loading ? "Loading..." : "Login"
                    }
                </button>
               
                <p className="w-full h-2 text-white text-right italic my-2 ">Don't have an account? click <Link to="/register" className="font-bold text-accent">Here</Link> </p>
                <button className="w-full h-[50px] bg-blue-900 mt-5 text-white rounded-lg flex justify-center items-center gap-2"><BsGoogle/> Sign In with Google</button>
            </div>
        </div>
    )
}
